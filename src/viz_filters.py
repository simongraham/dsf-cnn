...
def plot_filters(filter_list, alpha_list, beta_list):
    ...
    for i in range(len(filter_list)):
        ...
        plt.axis("off")
        plt.title(
            "Real: $alpha$= %s, $beta$= %s" % (alpha_list[i], beta_list[i]), fontsize=6
        )
        plt.subplot(len_x, len_y, count + 1)
        plt.imshow(filt_imag, vmin=-1, vmax=1, cmap=cm.gist_gray)
        plt.axis("off")
        plt.title(
            "Imaginary: $alpha$= %s, $beta$= %s" % (alpha_list[i], beta_list[i]),
            fontsize=6,
        )
        count += 2
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    if ksize not in [5, 7, 9, 11]:
        raise Exception("Select ksize to be either 5,7,9 or 11")
