...
def encoder(name, i, basis_filter_list, rot_matrix_list, nr_orients, filter_type, is_training):
    ...
    with tf.variable_scope(name):
        ...
        c2 = GConv2D('ds_conv2', c1, 10, 7, nr_orients, filter_type, basis_filter_list[1], rot_matrix_list[1])
...
def decoder(name, i, basis_filter_list, rot_matrix_list, nr_orients, filter_type, is_training):
    ...
    with tf.variable_scope(name):
        with tf.variable_scope('us1'):
            ...
            us1 = g_dense_blk('dense_us1', us1_sum, [14,6], [7,5], 4, nr_orients, filter_type, dense_basis_list, dense_rot_list)
        ...
        with tf.variable_scope('us3'):          
            ...
            us3 = g_dense_blk('dense_us3', us3_sum, [14,6], [7,5], 2, nr_orients, filter_type, dense_basis_list, dense_rot_list)
...
class Graph(Model):
    def _build_graph(self, inputs):
        ...
        with argscope(Conv2D, activation=tf.identity, use_bias=False, # K.he initializer
            ...
            o_logi_np = Conv2D('output_np', feat_np, 2, 1, use_bias=True, nl=tf.identity)
            ...
            o_logi_mk = Conv2D('output_mk', feat_mk, 3, 1, use_bias=True, nl=tf.identity)
        ...
        if get_current_tower_context().is_training:
            ...
            for term, weight in self.loss_term.items():
                if term == 'bce':
                    ...
                    term_loss_mk = tf.reduce_mean(term_loss_mk*pen_map, name='loss-bce-mk')
                elif 'dice' in self.loss_term:
                    # branch 1
                    term_loss_np = dice_loss(soft_np[...,0], one_np[...,0]) \
                              + dice_loss(soft_np[...,1], one_np[...,1])
                    term_loss_np = tf.identity(term_loss_np, name='loss-dice-np')

                    term_loss_mk = dice_loss(soft_mk[...,0], one_mk[...,0]) \
            ...
            viz = tf.concat([orig_imgs,
                                pred_np, pred_mk_blb, pred_mk_cnt,
                                true_np, true_mk_blb, true_mk_cnt], 2)
