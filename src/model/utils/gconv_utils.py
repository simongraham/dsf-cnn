...
def get_basis_params(k_size):
    """
    get the parameters for generating the basis
    filters.
    alpha_list: list of frequencies 
    beta_list: for Gaussian radial function: mean of ring
    """
    if k_size == 5:
        alpha_list = [0,1,2]
        beta_list = [0,1,2]
...
def get_basis_filters(alpha_list, beta_list, bl_list, k_size, eps = 10**-8):
    """ 
    Generate basis filters sampled on a KxK pixel grid with Gaussian radial profile. 
    Args:
        arguments
    Returns:
        returns
    ...
    filter_array = np.reshape(filter_array, [filter_array.shape[0], filter_array.shape[1], filter_array.shape[2], 1, 1, 1])
...
def gen_rotated_filters(w, filter_type, input_layer, nr_orients_out, basis_filters=None, rot_info=None):
    ...
    if filter_type == 'steerable':
        ...
        rot_filters = tf.stack(rot_filters) # [nr_orients_out, J, K, K, nr_orients_in, filters_in, filters_out] (M: nr frequencies, R: nr radial profile params)

        # Linear combination of basis filters
        rot_filters = tf.reduce_sum(rot_filters, axis=1) # [nr_orients_out, K, K, nr_orients_in, filters_in, filters_out]
        # Get real part of filters
        rot_filters = tf.math.real(rot_filters, name='filters') # [nr_orients_out, K, K, nr_orients_in, filters_in, filters_out]
    
    else:
        # if using regular kernels, rotate by sparse matrix multiplication

        filter_shape = w.get_shape().as_list() # [K, K, nr_orients_in, filters_in, filters_out]
    ...
    if input_layer is False: 
        ...
        rot_filters = cycle_channels(rot_filters, shape_list) # cycle channels - [nr_orients_out, K, K, nr_orients_in, filters_in, filters_out]
...
def GConv2D(
    ...
    if filter_type == 'steerable':
        ...
        w1 = tf.get_variable(name + '_W_real', w_shape, initializer=steerable_initializer(input_layer, nr_orients_out))
        w2 = tf.get_variable(name + '_W_imag', w_shape, initializer=steerable_initializer(input_layer, nr_orients_out))
        w = tf.complex(w1, w2)
        ...
        filters = gen_rotated_filters(w, filter_type, input_layer, nr_orients_out, basis_filters, rot_info)
    
    else:
        w_shape = [kernel_size, kernel_size, nr_orients_in, filters_in, filters_out]
        w = tf.get_variable(name + '_W', w_shape, initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out'))
    ...
    filters = tf.transpose(filters, [1, 2, 3, 4, 0, 5]) # [K, K, nr_orients_in, filters_in, nr_orients_out, filters_out]
    filters = tf.reshape(filters, [kernel_size, kernel_size, nr_orients_in * filters_in, nr_orients_out * filters_out])
    ...
    if use_bias:
        ...
        b = tf.get_variable(name + '_bias', [filters_out], initializer=tf.zeros_initializer()) 
