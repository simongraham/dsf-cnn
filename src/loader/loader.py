    ...
    def get_data(self):
        ...
        for idx in idx_list:
            ...
            if self.model_mode == 'seg_gland' or self.model_mode == 'seg_nuc':
                ...
                img = np.reshape(img, self.train_input_shape + [self.input_chans]) # reshape vector to HxWxC
...
#### 
def valid_generator_seg(ds, shape_aug=None, input_aug=None, label_aug=None, batch_size=16, nr_procs=1):
...
def valid_generator_class(ds, shape_aug=None, input_aug=None, batch_size=16, nr_procs=1):
    ### augment the input
    ds = ds if shape_aug is None else AugmentImageComponent(ds, shape_aug, index=0, copy=True)
    ### augment the input
    ds = ds if input_aug is None else AugmentImageComponent(ds, input_aug, index=0, copy=False)
    #
    ds = BatchData(ds, batch_size, remainder=True)
...
####
def train_generator_seg(ds, shape_aug=None, input_aug=None, label_aug=None, batch_size=16, nr_procs=8):
    ### augment both the input and label
    ds = ds if shape_aug is None else AugmentImageComponents(ds, shape_aug, (0, 1), copy=True)
