...
class InferClass(Config):
    ...
    def run(self):
        ...
        file_load_img = HDF5Matrix(self.inf_data_list[0] + 'camelyonpatch_level_2_split_test_x.h5', 'x')
        file_load_lab = HDF5Matrix(self.inf_data_list[0] + 'camelyonpatch_level_2_split_test_y.h5', 'y')
        ...
        for start_batch in range(0, last_step+1, self.inf_batch_size):
            ...
            if start_batch != last_step:
                ...
                lab = np.squeeze(file_load_lab[start_batch:start_batch+self.inf_batch_size])
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', help='choose infer mode- either "seg" or "class".')
